...
def main():
    ...
    if is_deepspeed_zero3_enabled:
        ...
        original_embeddings = model.get_input_embeddings()
        original_weights = original_embeddings.weight  # Access the weights tensor of the embeddings

        # Get the pre-expansion embeddings of the model and resize the embedding layer
        model.resize_token_embeddings(len(tokenizer) + len(special_tokens), pad_to_multiple_of=pad_factor)
        embeddings = model.get_input_embeddings()

        # Calculate the mean and covariance of the original embeddings
        mean = original_weights.mean(dim=0)
        n = original_weights.size(0)
        sigma = ((original_weights - mean).T @ (original_weights - mean)) / n

        # Define a multivariate normal distribution based on the computed mean and covariance
        dist = torch.distributions.multivariate_normal.MultivariateNormal(
            mean,
            covariance_matrix=1e-5 * sigma + 1e-5 * torch.eye(original_weights.size(1))  # Adding a small value to the diagonal for numerical stability
        )

        # Sample the embeddings for the new tokens
        new_token_embeddings = torch.stack(
            [dist.sample() for _ in range(len(special_tokens))],
            dim=0
        )

        # Assign the new embeddings to the embedding layer
        new_embeddings = torch.cat([original_weights, new_token_embeddings], dim=0)
